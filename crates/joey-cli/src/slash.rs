...
pub static REGISTRY: &[CommandDef] = &[
    ...
    cmd!("rollback", &[], "List or restore filesystem checkpoints", "Session", "[number]", false),
    cmd!("snapshot", &["snap"], "Create or restore state snapshots of Joey config/state", "Session", "[create|restore <id>|prune]", false),
    ...
    cmd!("agents", &["tasks"], "Show active agents and running tasks", "Session", "", false),
    ...
    cmd!("goal", &[], "Set a standing goal Joey works on across turns until achieved", "Session", "[text | draft <text> | show | pause | resume | clear | status]", false),
