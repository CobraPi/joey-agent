...
/// The complete TUI state. Cheap to clone-per-frame for borrowed rendering.
pub struct App {
    pub mode: RunMode,
    ...
    /// Reasoning visibility toggle (user can collapse with `r`).
...
impl App {
    ...
    /// Number of transcript rows, each pre-wrapped to a width for rendering.
    ...
    pub fn apply(&mut self, ev: AgentEvent) {
        match ev {
            ...
            AgentEvent::ContentDelta(d) => {
                if self.reasoning_open {
                    // Close the reasoning block and flush it to transcript.
            ...
            AgentEvent::AssistantMessage(text) => {
                // Flush any streaming text first, then commit the final block.
            ...
            AgentEvent::ToolProgress { name: _, progress } => {
                // Update the most recent running tool's summary.
                if let Some(TranscriptItem::Tool { summary, .. }) =
            ...
            AgentEvent::ToolEnd { name, is_error, result_preview, duration_secs } => {
                if let Some(TranscriptItem::Tool { name: n, status, duration_secs: dur, result_preview: rp, .. }) =
                    self.transcript.back_mut()
                {
                    if n == &name {
            ...
            AgentEvent::Done { final_text, usage, iterations } => {
                self.tokens.prompt += usage.prompt_tokens;
                self.tokens.completion += usage.completion_tokens;
                ...
                if !final_text.is_empty() {
                    self.push_item(TranscriptItem::Assistant { text: final_text.clone() });
                    self.last_final_text = final_text;
                } else if !leftover.is_empty() {
                    self.push_item(TranscriptItem::Assistant { text: leftover.clone() });
                    self.last_final_text = leftover;
                }
                if self.reasoning_open {
                    let text = std::mem::take(&mut self.streaming_reasoning);
                    if !text.is_empty() {
                ...
                for a in self.active_agents.iter_mut() {
        ...
        // Auto-scroll reset: any new event snaps to bottom unless user scrolled up.
        if self.scroll.is_some() && self.scroll == Some(0) {
            self.scroll = None;
        }
    }
    ...
    pub fn push_item(&mut self, item: TranscriptItem) {
        ...
        // Reset auto-follow on new content.
        self.scroll = None;
    }

    /// Record a user message in the transcript.
