...
- [X] T026 [US2] Add isolation verification to Subagent::run() in `crates/joey-orchestration/src/subagent.rs`: after run_turn completes, assert child.history() is independent — the returned DelegationResult.summary is the ONLY data that crosses to the parent; log a warning if summary exceeds 1000 tokens
