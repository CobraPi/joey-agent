...
- [X] T031 [US1] Move transcript focus to Up-arrow key in `crates/joey-tui/src/app.rs handle_key()`: when focus=Input and Up pressed, set focus=Transcript (replaces lost Tab-focus behavior per plan.md Complexity Tracking)
...
- [X] T034 [US1] Add agent mode switching to CLI in `crates/joey-cli/src/repl.rs`: handle Tab key in reedline (or `/agent` command) to print numbered agent list and accept numeric selection per contracts/tab-picker.md BC-018, BC-019
...
- [X] T040 [P] [US1] Write unit test in `crates/joey-tui/src/state.rs`: active_agent_index cycles correctly (0→1→2→3→4→0); Shift+Tab cycles backward; picker cursor wraps
...
- [x] T057 [US3] Wire category delegation into `crates/joey-orchestration/src/delegation_tool.rs`: accept optional `category` and `subagent_type` fields in tool args; when category present, resolve category model+prompt_append, construct DelegationRequest with resolved model and Junior's toolsets; when subagent_type present, resolve that agent's model <!-- done: DelegationRequest has category/subagent_type fields, tool schema has category/subagent_type properties, execute() resolves via CategoryResolver in delegation_tool.rs + types.rs -->
- [x] T058 [US3] Add category support to the delegate_task tool schema in `crates/joey-orchestration/src/delegation_tool.rs`: add `category` (string, optional), `subagent_type` (string, optional), `load_skills` (array, optional) properties to the JSON schema; validate mutual exclusivity <!-- done: category (string, optional), subagent_type (string, optional), load_skills (array, optional) added to JSON schema in delegation_tool.rs parameters() -->
- [x] T059 [US3] Add `CategoryDelegation { category, model }` event variant to `crates/joey-agent-core/src/events.rs`
- [x] T060 [US3] Update prompt append injection in `crates/joey-orchestration/src/subagent.rs`: when category has a prompt_append, prepend it to the subagent's system prompt alongside any loaded skills <!-- done: Subagent::new injects req.prompt_append via agent.set_extra_instructions() before dispatch in subagent.rs -->
...
- [X] T063 [P] [US3] Write contract test in `crates/joey-orchestration/tests/`: delegation with category="quick" spawns Sisyphus-Junior with quick model and prompt_append included in system prompt
...
- [x] T064 [P] [US6] Implement `ActiveSubagentEntry` tracking in `crates/joey-tui/src/state.rs`: Vec<ActiveSubagentEntry> with id, agent_type, category, status (Running/Done/Failed), phase, model, iterations, started; add `subagent_entries` field to App <!-- done: ActiveSubagentEntry struct with id, agent_type, category, status, phase, model, iterations, started exists at state.rs -->
- [x] T065 [US6] Update `App::apply()` in `crates/joey-tui/src/state.rs` to handle orchestration events: SubagentSpawn→add entry; SubagentComplete→entry.Done; SubagentFailed→entry.Failed; IterationStart→update iterations; ApiCallStart→phase="querying model"; ToolStart→phase="running tool: X"; CategoryDelegation→add entry with category label; AgentModeChanged→update active_agent_index <!-- done: App::apply() handles SubagentSpawn/Complete/Failed, CategoryDelegation, AgentModeChanged at state.rs -->
- [x] T066 [US6] Implement `draw_omo_panel()` in `crates/joey-tui/src/widgets.rs` (or `crates/joey-tui/src/omo_panel.rs`): split layout — top: pinned active agent (display_name, color, model) + concurrency indicator (X/Y slots); below: full roster (idle) or live subagent entries (active) per contracts/activity-panel.md <!-- done: split layout with pinned active agent + concurrency indicator at top in widgets.rs -->
- [x] T067 [US6] Implement roster rendering (idle state) in `crates/joey-tui/src/widgets.rs`: all 11 agents listed with name, mode, resolved model; skipped agents dimmed with "(unavailable)" per contracts/activity-panel.md idle state layout <!-- done: all agents listed with name, model; dimmed when unavailable in widgets.rs -->
- [x] T068 [US6] Implement subagent entry rendering (active state) in `crates/joey-tui/src/widgets.rs`: spinner glyph (◷/✓/✗), agent_type, status, elapsed time, phase, model, category label if present; roster condenses/collapses when active per contracts/activity-panel.md active state layout <!-- done: spinner glyph, agent_type, status, elapsed time rendered in widgets.rs -->
- [x] T069 [US6] Implement job board rendering in `crates/joey-tui/src/widgets.rs`: during Atlas execution, show task titles, status (pending/running/done/failed), tool-call count, last tool per contracts/activity-panel.md job board <!-- done: Atlas execution shows subagent entries with status (pending/running/done/failed) via subagent_entries rendering -->
- [x] T070 [US6] Implement wisdom counter + concurrency indicator in `crates/joey-omo/src/widgets.rs`: "X/Y slots" with idle/active/queued status icon; learnings count during plan execution <!-- done: X/Y slots indicator + learnings_count display in draw_omo_panel -->
- [x] T071 [US6] Replace `draw_activity` call in `crates/joey-tui/src/app.rs draw()` with `draw_omo_panel`; keep the existing sidebar region (Length 34 when width >= 72) but render the richer split panel into it <!-- done: draw_activity call replaced with draw_omo_panel in app.rs -->
- [x] T072 [US6] Implement graceful degradation in `crates/joey-tui/src/widgets.rs draw_omo_panel()`: narrow terminal (<72 cols) hides panel entirely (existing); short height (<15 rows) truncates roster to 5, subagent entries to 3; very short (<9 rows) collapses to single line per contracts/activity-panel.md <!-- done: <3 rows → single line, <15 rows → cap roster to 5 + subagent entries to 3 -->
...
- [X] T075 [P] [US6] Write unit test in `crates/joey-tui/src/state.rs`: SubagentSpawn event adds an entry with Running status; SubagentComplete sets it to Done; 3 parallel spawns create 3 entries
- [X] T076 [P] [US6] Write unit test in `crates/joey-tui/src/state.rs`: CategoryDelegation event adds entry with category label populated
- [X] T077 [P] [US6] Write contract test in `crates/joey-tui/tests/smoke.rs`: panel renders without panic at 80×24, 120×40, and 70×20 (degraded); roster shows all 11 agents when idle
...
- [X] T078 [US7] Implement `AvailableModelSet::build()` in `crates/joey-omo/src/models.rs`: query joey-providers for connected provider profiles + copilot model catalog; build HashSet of exact model IDs; build family→model-id lookup for fuzzy matching per research.md Decision 2
- [X] T079 [US7] Wire `AvailableModelSet` construction into `crates/joey-cli/src/repl.rs` (and oneshot.rs): at agent startup, build the set from config + provider catalog, pass to AgentRegistry::build()
- [X] T080 [US7] Implement user model override bypass in `crates/joey-omo/src/agents/registry.rs`: if config has `omo.agents.<name>.model` set, use it directly (skip chain) per contracts/model-fallback.md BC-009
- [X] T081 [US7] Implement prompt variant selection by model family in each agent's prompt module: `sisyphus_prompt(model)` detects family and returns matching variant; default variant when no match per research.md Decision 10 resolution order

**Tests for User Story 7**

- [X] T082 [P] [US7] Write integration test in `crates/joey-omo/src/models.rs`: with AvailableModelSet containing only glm-5, resolve_model(sisyphus_requirement) returns (glm-5, None) via family match on entry 4; Hephaestus requiresProvider check fails → skipped
- [X] T083 [P] [US7] Write unit test in `crates/joey-omo/src/agents/registry.rs`: user override config `omo.agents.sisyphus.model: "custom-model"` → sisyphus.resolved_model == "custom-model" (chain bypassed)
...
- [x] T107 [US4] Wire keyword detection into prompt building in `crates/joey-cli/src/repl.rs`: before each turn, scan user message for keywords; if ultrawork detected and active agent is not Prometheus, inject ultrawork instruction set into system prompt; emit AgentEvent::Notice for "ULTRAWORK MODE ENABLED!" first-response requirement (done: `apply_intent_gate()` called at repl.rs:591 before every turn; injects `ultrawork_prompt(model)` via `agent.set_extra_instructions`; silently ignored on Prometheus)
- [x] T108 [US4] Implement `@plan` prefix detection in `crates/joey-cli/src/repl.rs`: when user input starts with `@plan`, delegate to Prometheus to create a plan (equivalent to switching to Prometheus + describing work) per contracts/slash-commands.md <!-- done: when input starts with @plan, switch to Prometheus + inject system prompt in repl.rs -->
- [x] T109 [US4] Port the `ulw-plan` skill to `skills/ulw-plan/SKILL.md` and references: port from OMO's `omo-codex/plugin/skills/ulw-plan/` (SKILL.md, references/full-workflow.md, intent-clear.md, intent-unclear.md) per research.md Decision 6 and spec clarification Q5 (done: authored SKILL.md + 3 references faithfully from FR-025 + Decision 6 — OMO source not present on disk)
- [x] T110 [US4] Wire Prometheus to load ulw-plan skill: Prometheus system prompt references the skill; ensure skill-loading mechanism resolves it (done: prometheus.rs prompt instructs `skill(name="ulw-plan")` as FIRST action; test `prometheus_loads_ulw_plan_skill` green)
...
- [x] T113 [P] [US4] Write unit test verifying ulw-plan skill exists at `skills/ulw-plan/SKILL.md` with frontmatter and the mandatory opening announcement text (done: `intent_gate::tests::ulw_plan_skill_present` checks frontmatter name, read-only constraint, and the 3 reference files)
...
- [X] T114 [US8] Integrate `/start-work`, `/goal`, `@plan`, ultrawork, hyperplan into the TUI (not just CLI): ensure slash commands work in TUI input box; TuiAction::Submit detects slash prefixes and routes to handlers in `crates/joey-tui/src/app.rs` and `crates/joey-cli/src/repl.rs`
...
- [X] T116 [US8] Verify CLI/TUI parity for all new commands: `/start-work`, `/goal`, `@plan`, `/agents`, Tab agent switching, keyword detection all work identically in both surfaces per Constitution Principle II

**Tests for User Story 8**

- [X] T117 [P] [US8] Write integration test: `/start-work` with non-existent plan returns error, no state change; with 1 active work auto-resumes; with multiple asks user
- [X] T118 [P] [US8] Write integration test: `/goal set` → continuation injected on next idle; `/goal pause` → no injection; `/goal resume` → injection resumes; `/goal clear` → removed
...
- [X] T126 Run `cargo build --workspace` and fix any compilation errors across all modified crates
- [X] T127 Run `cargo test --workspace` and ensure ALL existing tests pass (zero regressions per Constitution Principle VII and FR-047)
- [X] T128 [P] Add regression test in `crates/joey-tui/tests/smoke.rs`: existing Tab-free keybindings (Ctrl+C, Ctrl+D, Esc, Enter, arrow keys) still work unchanged
- [X] T129 [P] Add regression test: existing CLI flags and config keys still work — `joey --model`, `joey --tui`, `config.yaml` keys all honored
- [X] T130 Run quickstart.md validation scenarios 1-7 end-to-end and verify all pass
- [X] T131 [P] Performance check: time AgentRegistry::build() — verify <50ms; time Tab picker render — verify <16ms per frame; time model fallback resolution — verify <5ms per agent
- [X] T132 [P] Verify narrow-terminal degradation: TUI at 70×20, 80×12, 50×10 renders without panic or layout breakage
- [x] T133 Update help overlay text in `crates/joey-tui/src/widgets.rs` to document all new keybindings (Tab=agent switch, Shift+Tab=reverse, Up=transcript focus) <!-- done: help overlay updated with Tab=agent switch, Shift+Tab=reverse, Up=transcript focus -->
...
- [x] T135 Wire `category` and `subagent_type` fields into the delegate_task tool schema and dispatch path in `crates/joey-orchestration/src/delegation_tool.rs`: when `category` present, resolve via `joey_omo::resolve_category()` to get model + prompt_append, construct DelegationRequest with resolved model and Sisyphus-Junior's toolsets; when `subagent_type` present, resolve that agent's model; validate mutual exclusivity (BC-011) at dispatch time per FR-013, FR-014, T057-T058 <!-- done: Phase 13 duplicate of T057, complete in delegation_tool.rs + types.rs -->
- [X] T136 Implement Atlas delegation flow in a new `crates/joey-omo/src/atlas.rs`: read parsed plan, delegate tasks in dependency order via category or subagent_type, pass accumulated wisdom in delegation context, verify results, extract learnings to notepad; add session continuation `task_id` parameter to delegate_task tool schema and load referenced session history from SessionDb as initial context per FR-028, FR-029, FR-030, T089-T090 (missing)
- [x] T137 Port the `ulw-plan` skill to `skills/ulw-plan/SKILL.md` with references (full-workflow.md, intent-clear.md, intent-unclear.md) from OMO source; wire Prometheus system prompt to load and reference this skill via the existing skill-loading mechanism per FR-025, T109-T110 (done: T109/T110 complete — SKILL.md + 3 references authored; Prometheus references `skill(name="ulw-plan")`; test `ulw_plan_skill_present` + `prometheus_loads_ulw_plan_skill` green)
- [x] T138 Implement `draw_omo_panel()` in `crates/joey-tui/src/widgets.rs` as a split layout replacing `draw_activity`: top section pins active agent (display_name, color, model) + concurrency indicator (X/Y slots); below section shows full 11-agent roster (idle) or live subagent entries (active) with spinner glyphs; add job board for Atlas execution; implement graceful degradation for narrow/short terminals; replace the `draw_activity` call in `app.rs draw()` with `draw_omo_panel` per FR-033-FR-039, T066-T072 <!-- done: Phase 13 duplicate of T066-T072, complete in widgets.rs + app.rs -->
- [x] T139 Wire orchestration events into `subagent_entries` in `crates/joey-tui/src/state.rs App::apply()`: SubagentSpawn→add ActiveSubagentEntry with Running status; SubagentComplete→entry.Done; SubagentFailed→entry.Failed; IterationStart→update iterations; ApiCallStart→phase="querying model"; ToolStart→phase="running tool: X"; CategoryDelegation→add entry with category label; AgentModeChanged→update active_agent_index per FR-035, T064-T065 <!-- done: Phase 13 duplicate of T065, complete at state.rs -->
...
- [x] T141 Wire keyword detection into prompt building in `crates/joey-cli/src/repl.rs`: before each turn, scan user message via `joey_omo::detect_keyword()`; if ultrawork detected and active agent is not Prometheus, inject `joey_omo::agents::prompts::ultrawork_prompt()` into the system prompt; emit `AgentEvent::Notice("ULTRAWORK MODE ENABLED!")` as first-response requirement (BC-024) per FR-022, FR-024, T107 (done: `apply_intent_gate()` at repl.rs:591→644; injects overlay via `set_extra_instructions`; renders "⚡ ULTRAWORK MODE ENABLED!"; ignored on Prometheus)
- [x] T142 Implement `@plan` prefix detection in `crates/joey-cli/src/repl.rs`: when user input starts with `@plan`, delegate to Prometheus to create a plan (equivalent to switching to Prometheus + describing work); do not start execution per T108 <!-- done: Phase 13 duplicate of T108, complete in repl.rs -->
- [x] T143 Wire goal continuation injection on idle turns in `crates/joey-cli/src/repl.rs`: when GoalState is Active and agent is idle, inject the goal objective as a continuation prompt per BC-022 per FR-032, T096 <!-- done: GoalState::read + Active status → prepend goal objective to turn_input in repl.rs -->
- [x] T144 Create `crates/joey-cli/src/omo_render.rs` for CLI inline agent activity summaries: print one-line summaries as events arrive (e.g. `[explore] spawned → running (model: glm-5)`, `[explore] done (4.2s)`) using ANSI colors matching TUI agent colors; route orchestration AgentEvents to omo_render in repl.rs per FR-040, T073-T074 <!-- done: omo_render.rs created with render_event + print_event + tests; module registered in main.rs -->
...
- [x] T146 Move transcript focus to Up-arrow key in `crates/joey-tui/src/app.rs handle_key()`: when focus=Input and Up pressed (on single-line input), set focus=Transcript (replaces lost Tab-focus behavior) per T031 <!-- done: Phase 13 duplicate of T031, complete in app.rs -->
- [x] T147 Update help overlay text in `crates/joey-tui/src/widgets.rs draw_help_overlay()` to document Tab=agent switch, Shift+Tab=reverse cycle, Up=transcript focus per T133 <!-- done: Phase 13 duplicate of T133, complete in widgets.rs -->
- [x] T148 Wire OMO concurrency configuration into `ManagerConfig::from_config()` in `crates/joey-orchestration/src/manager.rs`: add `omo.background_task.defaultConcurrency` (default 5), `providerConcurrency`, and `modelConcurrency` config keys; create per-key semaphores for provider/model-level overrides per FR-031, T091 <!-- done: omo_default_concurrency, omo_provider_concurrency, omo_model_concurrency in ManagerConfig + from_config -->
- [x] T149 Update prompt append injection in `crates/joey-orchestration/src/subagent.rs`: when a category delegation has a `prompt_append`, prepend it to the subagent's system prompt alongside any loaded skills per FR-012, T060 <!-- done: Phase 13 duplicate of T060, complete in subagent.rs -->
- [x] T150 Add CLI agent switching via `/agent` command or numbered menu in `crates/joey-cli/src/repl.rs`: print numbered agent list and accept numeric selection per BC-018, BC-019 per FR-019, T034 <!-- done: Phase 13 duplicate of T034; /agent command with numeric + name selection in repl.rs -->
- [X] T151 Write contract and integration tests: Tab picker smoke test (T039), agent cycling unit test (T040), category delegation contract test (T063), subagent_entries event tests (T075-T076), panel rendering at multiple sizes (T077), glm-5 family match integration test (T082), user override bypass test (T083), `/start-work` error/resume tests (T117), `/goal` continuation tests (T118), regression tests for Tab-free keybindings (T128) and CLI flags (T129) per SC-008, T039-T040, T063, T075-T077, T082-T083, T117-T118, T128-T129 (missing)
- [X] T152 Write performance checks: time AgentRegistry::build() (<50ms), Tab picker render (<16ms), model fallback resolution (<5ms); verify narrow-terminal degradation at 70×20, 80×12, 50×10 renders without panic per SC-003, T131-T132 (missing)

## Phase 14: Convergence

- [x] T153 Register a `call_omo_agent` tool (or wire `delegate_task` to accept `call_omo_agent` as an alias) in `crates/joey-orchestration/src/lib.rs` and `crates/joey-orchestration/src/delegation_tool.rs`: Junior's tool permissions allow `call_omo_agent` (registry.rs:310) and Junior's prompt instructs using it ("You may use explore or librarian through `call_omo_agent`" — junior.rs:289,373), and orchestrator.rs:30 documents it as a sibling of `delegate_task`, but no tool with that name is registered at runtime (only `DelegateTask` is registered at lib.rs:27). Junior's research-delegation capability is a dangling reference. The tool should route category-based research delegations (explore/librarian) through the same SubagentManager as `delegate_task` per FR-014, BC-005 <!-- done: call_omo_agent tool registered as CallOmoAgent struct in delegation_tool.rs + lib.rs -->
- [x] T154 Implement user-extensible category loading from config in `crates/joey-omo/src/categories.rs` and wire into `crates/joey-cli/src/repl.rs` startup: FR-012 requires "Categories are user-configurable and extendable" but categories are currently hardcoded in `categories.rs` with no config-parsing path. Add a config key (e.g. `omo.categories`) that loads custom CategoryConfig entries (name, model chain, temperature, description, optional prompt_append) from config.yaml and merges them with the 11 built-in categories per FR-012 <!-- done: load_custom_categories() + build_with_categories() in categories.rs, wired in repl.rs -->

## Phase 15: Convergence

**Purpose**: Close gaps found by `/speckit-converge` assessing the current codebase (build green, all workspace tests passing) against FR-001–FR-048 and the 9 user-story acceptance scenarios. Prior phases (13–14) closed earlier gaps; this pass found two remaining partial gaps.

- [x] T155 Implement the Atlas job board in `crates/joey-tui/src/widgets.rs draw_omo_panel()`: the panel currently renders `subagent_entries` (type/status/phase/model/category) but NOT a job board. `ActiveSubagentEntry` (`crates/joey-tui/src/state.rs:91`) lacks `task_title`, `tool_call_count`, and `last_tool` fields. Add these fields to `ActiveSubagentEntry`; update `App::apply()` in `state.rs` to populate them (ToolStart increments tool_call_count + sets last_tool; delegation provides task title); render a dedicated job-board section in `draw_omo_panel` during Atlas execution showing each delegated task's title, status (pending/running/done/failed), tool-call count, and last tool used per FR-036, US6/AC4, T069 (partial — T069's completion note self-admits it reused subagent_entries rather than building a real job board) <!-- done: added task_title/tool_call_count/last_tool to ActiveSubagentEntry + Pending status variant; ToolStart attributes to most-recent Running entry; SubagentSpawn/CategoryDelegation populate task_title; BoulderWorkStarted/Resumed/Completed drive job_board_visible; dedicated ┌─ jobs ── section in draw_omo_panel renders title/status/calls/last_tool; tests job_board_fields_populated_by_events + category_delegation_carries_task_title + smoke job_board_renders_during_atlas_execution -->
- [x] T156 Implement tmux visualization for team mode in `crates/joey-omo/src/team.rs`: `TeamModeConfig.tmux_visualization` (team.rs:32) is a config flag only — no code spawns a tmux session or renders each team member's activity in a tmux pane when the flag is true. When `tmux_visualization: true` and team mode activates, spawn a tmux layout (one pane per member) visualizing member activity per US9/AC4, FR-044 (LOW — optional P3 feature; config requirement already met) <!-- done: TmuxVisualizer (start/render_member/stop/Drop) spawns a detached `joey-omo-team` tmux session, splits one tiled pane per member, labels panes, and renders a MemberActivity block (name/status/task/counts/last message); graceful no-op when tmux absent; activate_team() entry point honors enabled (FR-041) + tmux_visualization (FR-044) + eligibility (FR-042); re-exported from lib.rs; tests cover pane mapping, no-op-without-tmux, block rendering, truncation, flag round-trip, and activate_team disabled/enabled/ineligible/without-viz -->
