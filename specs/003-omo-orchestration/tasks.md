...
- [ ] T039 [P] [US1] Write contract test in `crates/joey-tui/tests/smoke.rs`: Tab key opens agent picker; Enter selects; agent_roster updated; status bar reflects selection
...
- [ ] T145 Update `draw_status()` in `crates/joey-tui/src/widgets.rs` to show the active agent display_name + color indicator from `app.agent_roster[app.active_agent_index]` per FR-017, T036 (partial)
