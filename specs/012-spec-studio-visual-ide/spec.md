...
- Q: What concrete performance budget should anchor SC-010 and FR-040, which currently self-reference a "stated performance budget" without stating one? → A: Adopt the concept's measured targets verbatim: initial board render ≤400 ms for 200 tasks, markdown parse ≤400 ms for 200 tasks, 60 fps interaction. These are technology-agnostic (units are ms/fps, not stack names), directly testable, and justified by the concept's own measurements (parse-on-demand measured 1.2 s p95 — 3× over budget — which is the architectural justification for the P0 lossless parser + semantic cache).
- Q: What persistence backend should the Overlay layer (FR-032) use for private IDE state (run history, private bindings, anchored comments, board positions, filters, panel layout)? → A: Extend the existing `~/.joey/speckit-ui/` convention established in `specs/010`: append-only JSONL for log records (run history, accepted clarify answers, anchored-comment threads) keyed by feature, plus a small per-repo+branch JSON key/value file for genuinely mutable UI state (board positions, filters, panel layout, open artifacts). Adds zero new dependencies (Constitution VIII), keeps every Overlay record text-readable and diffable, avoids introducing a second schema/versioned format into `joey-speckit-ui` (Constitution VII), and naturally extends the `specs/010` JSONL precedent rather than forking it. A SQLite overlay store is rejected as unjustified given the JSONL+JSON fit and the constitution's dependency-justification bar.
- Q: For the four defect classes in FR-023, should the "one-click fix" produce a deterministic template stub or delegate to the agent for a reviewed, staged patch? → A: Hybrid. The one-click fix performs the deterministic structural scaffolding itself (inserting a correctly-anchored, correctly-attributed stub at the right byte range — e.g. a stub task line owning the orphan requirement), which is instant, free, and reproducible, honoring the concept's "deterministic wherever computable" principle. The genuinely generative part (filling in a real task body, drafting a real constitution-breach justification) is offered as a follow-on that routes through the agent under the same staged-review policy as every other agent edit (FR-025/FR-029). This keeps free fixes free, reserves the model for where it adds real value, and stays consistent with the staging and review contract.
...
- **FR-023**: The IDE MUST detect and offer one-click fixes for four defect classes: orphan requirements (generate the missing task), rogue tasks (link to a requirement or promote), unverified implemented items (feed into checklist generation), and constitution breaches (justify or redesign). Each one-click fix MUST use a hybrid model: it performs the deterministic structural scaffolding itself — inserting a correctly-anchored, correctly-attributed stub at the right byte range (e.g. a stub task line owning the orphan requirement, or a stub checklist item for an unverified task), which is instant, free, and reproducible — and then offers the genuinely generative follow-on (filling in a real task body, drafting a real breach justification) as a scoped agent run that produces a reviewed, staged patch under the same policy as every other agent edit (FR-025/FR-029). The cross-artifact consistency analysis MUST render as a persistent live matrix rather than a one-shot prose report.
...
- **FR-032**: The system MUST be organized as three layers: Truth (markdown, CLI-compatible sidecars, and git — written only by the agent through reviewed patches or the developer's accepted edits, with unknown syntax preserved and untouched ranges never rewritten), Meaning (a lossless concrete syntax tree plus a derived semantic graph where every node keeps its byte range, expected bytes, revision hash, and structural fingerprint), and Overlay (private IDE state — run history, private bindings, anchored comments, board positions, filters, panel layout — that MUST never dirty the working tree). Shared binding metadata is a separate, explicit repo-sidecar mode that the IDE previews before enabling. The Overlay layer MUST persist under the existing `~/.joey/speckit-ui/` data directory, extending the `specs/010` convention: append-only JSONL files (one per feature, at `~/.joey/speckit-ui/history/<feature-id>.jsonl`) for log records — run history, accepted clarify answers, anchored-comment threads — and a small per-repo+branch JSON key/value file for genuinely mutable UI state — board positions, filters, panel layout, open artifacts. This adds no new runtime dependency (Constitution VIII) and introduces no new schema/versioned database format alongside the existing JSONL convention (Constitution VII); a SQLite overlay store is rejected as unjustified.
...
- **FR-040**: The IDE MUST remain interactive for large features — including boards of at least 200 tasks — within the performance budget: an initial board render ≤400 ms for 200 tasks, a markdown parse ≤400 ms for 200 tasks, and 60 fps sustained interaction, without hiding records or preventing completion of the primary workflow. (These targets are the architectural justification for the P0 lossless parser plus a derived semantic cache: parse-on-demand was measured at 1.2 s p95 — 3× over budget — so the cache is a justified complexity, not speculative generality.)
...
- **SC-010**: A board of 200 tasks renders its initial view in ≤400 ms, a 200-task markdown parse completes in ≤400 ms, and sustained interaction (filtering, toggling, scrolling) holds 60 fps for at least 95% of measured interactions.
