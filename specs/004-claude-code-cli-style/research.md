...
- **Feature flags**: `joey-cli` uses `pulldown-cmark = { workspace = true }` with DEFAULT features only. `simd` (faster parsing via SIMD) is unnecessary for single-message terminal reflows at human typing cadence and would add build complexity; `gen` (code generation) is irrelevant. Binary-size impact is negligible because the crate is already compiled into the workspace via `joey-speckit-ui` (no new transitive surface).
...
## T043 Decision: FR-007 Resize Detection — Lazy Width Re-Read (Chosen Approach)

**Date**: 2026-07-25 (convergence phase)

**Finding**: The spec (FR-007, T033) called for a `crossterm::event::Resize` arm in the `tokio::select!` tick loop. After analysis, adding a blocking `event::read()` via `spawn_blocking` to the async select! loop introduces complexity and a race condition risk: the blocking read consumes ALL crossterm events (key presses, mouse, etc.), not just resize, which would interfere with reedline's own event loop on the same stdin.

**Chosen approach**: Lazy width re-read via `box_width()` on every render call. This is the standard approach for line-based CLIs (as opposed to full-screen TUIs which need immediate resize redraws). `box_width()` calls `terminal_size::terminal_size()` on each invocation, so the layout adapts to the new width on the next render without any event polling.

**Rationale**: 
1. Line-based CLIs don't maintain a screen buffer to redraw — each `println!`/`print!` is append-only, so there's no "partial frame" to repair on resize.
2. The only width-sensitive elements are `box_width()` (reasoning box, banner scaling) and the markdown renderer (wrapping), both of which re-read width on each call.
3. Adding `event::read()` would require either a separate thread (conflicts with reedline) or `spawn_blocking` (adds latency to the tick loop and consumes all input events).
4. The edge case "terminal resized during animation" is handled: the spinner repaints on each tick using `\r` + `Clear(CurrentLine)`, which is width-agnostic; the markdown reflow on Done reads the current width at that moment.

**Trade-off**: There is no IMMEDIATE relayout when the terminal is resized mid-streaming (the current delta continues at the old width until the next delta). This is acceptable for a line-based CLI and matches the behavior of most terminal CLIs.

## T031 Spike Decision: US6 Prompt Caret Blink — DESCOPED

**Date**: 2026-07-25 (implementation phase)

**Finding**: reedline (v0.40) owns a synchronous, blocking editor loop (`Repl::read_line`). The `Prompt` trait's `render_prompt_*` methods are called only on reedline's internal redraws (keystroke-driven), not on an external timer. reedline provides no `Highlighter` or prompt-render hook that fires on an external tick.

**Impact**: An external tick-driven prompt blink (FR-010 compliance for US6) would require a separate background thread writing cursor-control escapes to stdout while reedline blocks on stdin. This would corrupt reedline's screen rendering — an unacceptable regression risk (Constitution VII).

**Decision**: DESCOPE US6 to a static colored prompt (no blink). The prompt color is upgraded from `DarkGrey` to `Cyan` (approximating Pantera `info`) for a more polished look within reedline's ANSI-16 `Color` enum. The `PromptCaret` profile data remains registered in `profile.rs` for future use if reedline adds external-tick support or is replaced.

**Constitution compliance**: Principle VII (Non-Regression) takes priority over the US6 nice-to-have. The descope is explicitly permitted by the T031 task definition.
