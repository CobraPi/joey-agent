...
**Branch**: `004-claude-code-cli-style` | **Date**: 2026-07-24 | **Spec**: [spec.md](spec.md)

**Input**: Feature specification from `/specs/004-claude-code-cli-style/spec.md`
...
### Principle VI — Modularity and Decoupling

**Crate(s) touched**: `joey-cli` only (same as Principle I analysis).

**Narrow interface exposed**: All new animation modules expose `pub(crate)` items only. The single cross-module public entry points consumed by `repl.rs` are `render::banner_animated` and the unchanged `render::render_turn` signature. `RenderOptions` gains three plain-data fields consumed internally. No sibling crate depends on a new internal of another.

**Acyclic, minimized coupling**: Dependency direction unchanged (`joey-core` → `joey-agent-core` → `joey-cli`). A change in `joey-cli`'s animation code does not force edits to any other crate.

**Verdict**: PASS. Narrow explicit interfaces; no new cross-crate coupling.

### Principle VII — Backward Compatibility and Non-Regression (NON-NEGOTIABLE)

**Public surfaces touched**:
- `render::render_turn` signature: UNCHANGED (`pub async fn render_turn(rx, opts) -> String`). Refactor is internal.
- `RenderOptions`: gains three ADDITIVE fields (`animations_enabled`, `animation_fps`, `capability`). Existing fields (`show_reasoning`, `tool_progress`, `quiet`) unchanged. `RenderOptions` is constructed only inside `joey-cli` (`repl.rs`) and derives `Clone`; the additions are internal and non-breaking.
- CLI flags / config keys / exit codes / on-disk formats: UNCHANGED. `display.animation_fps` is a NEW config key (additive).

**Regression coverage mandated**: tasks.md MUST include a characterization test asserting the pre-feature `render_turn` output shape (plain-text path) is preserved for a fixed `AgentEvent` stream. → Satisfied by tasks.md T035a.

**Verdict**: PASS, conditional on the regression-coverage task (T035a) being present and green in tasks.md.

### Principle VIII — Performance Discipline and Lean Code

**Performance-sensitive path identified**: the refactored `render_turn` tick loop (now multiplexes `rx.recv()` with `tokio::time::interval`). This runs on every turn and every streamed token.

**Budget**:
- Tick rate: 12 fps default (~83 ms interval). Idle overhead: tick timer dormant except during an active turn/banner.
- Streaming latency: `ContentDelta` MUST print immediately (no added latency on the hot path; tick only repaints caret/spinner).
- Markdown finalize: single bounded reflow on completion (one message).
- CPU overhead when idle: negligible.

**Dependency weight** (pulldown-cmark 0.12): already a workspace dep used by `joey-speckit-ui`; promoted, not newly introduced. Default features only; binary size impact negligible (shared across the workspace already).

**Verdict**: PASS. Budget recorded; lean (zero-copy N/A for terminal output; single reflow; dormant timer when idle).

### Pre-design Gate Result: PASS (all eight principles). No violations to track.
Condition: regression-coverage task (T035a) present in tasks.md.
...
```text
specs/004-claude-code-cli-style/
├── plan.md              # This file
...
No violations. Table intentionally empty — all eight principles PASS at pre-design gate. Re-evaluated after Phase 1 design in the Completion Report.
