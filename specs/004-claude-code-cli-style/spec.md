...
**Feature Branch**: `004-claude-code-cli-style`
...
- **FR-005**: The CLI MUST show a persistent token-count usage indicator (prompt/completion tokens) that reflects in-flight usage during a turn, and a turn-complete summary line (tokens used, turn duration) after each response, rendered in Crush/Pantera colors. The summary MUST NOT overwrite or interfere with the streamed response text.
...
- A reasonable default frame rate (a ~12 fps tick loop, ~83 ms interval, with per-element intervals) will be chosen and made configurable via `display.animation_fps`; exact frame timing is an implementation detail.
