...
_Appended by /speckit-converge on 2026-09-13 (round 3) — latent lifecycle defect surfaced by re-assessment after T034; existing suites stay green because no test runs a wave and then a runaway child on one manager tree._

- [X] T035 Stop the shared watchdog kill-by-transient-drop: `impl Drop for SubagentManager` (crates/joey-orchestration/src/manager.rs ~:2746-2764) sets the tree-shared `gov_shutdown` flag unconditionally, but transient child managers (`shared_child_manager` ~:914 and the dispatch_requests batch-wave literal) clone the same Arc — so the first completed batch/background wave terminates the tree's watchdog for the rest of the session, silently disabling CPU-ceiling enforcement (FR-010) and cpu/memory attribution (FR-011, plan R5b); fix by adding a `gov_watchdog_owner: bool` field (true only in `SubagentManager::new`, false in every transient/test-helper literal), gating the flag store in Drop on ownership, with an in-module unit test (transient drop leaves shutdown false; owner drop sets it) and a governance_waves regression test (batch wave first, then an infinite-compute child on the same manager still aborted at cpu_ceiling_secs with outcome aborted_by_resource_limit) (partial)
