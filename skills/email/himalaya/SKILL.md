...
This skill is separate from the Joey Email gateway adapter. The gateway
adapter lets people email the agent and uses Joey' built-in IMAP/SMTP
...
To reply non-interactively from Joey, read the original message, compose a reply, and pipe it:
...
**Non-interactive (use this from Joey)** — pipe the message via stdin:
