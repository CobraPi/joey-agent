...
# Joey Desktop Plugins Skill

Write plugins for the Joey desktop app: statusbar items, layout panes,
...
system (`~/.joey/plugins/`) is otherwise documented separately.
...
- The Joey desktop app (it loads plugins; the CLI/gateway alone does not).
- Write access to `$JOEY_HOME/desktop-plugins/` (usually
  `~/.joey/desktop-plugins/`).

## How to Run

1. Create `$JOEY_HOME/desktop-plugins/<name>/plugin.js` from
   `templates/plugin.js` (relative to this skill directory) — that's
   `~/.joey/...` by default, or `~/.joey/profiles/<profile>/...` under a
...
The ONLY import surface is `@joey/plugin-sdk` (plus `react` /
...
- Backend: if the plugin ships a Python `plugin_api.py` (under
  `~/.joey/plugins/<id>/dashboard/`, manifest `"api": "plugin_api.py"`), reach
...
- Do not import anything except `@joey/plugin-sdk`, `react`, and
