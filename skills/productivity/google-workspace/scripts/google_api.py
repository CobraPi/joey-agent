#!/usr/bin/env python3
"""Google Workspace API CLI for Joey Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Joey-facing JSON contract and falls back to the Python client
...
from _joey_home import get_joey_home
...
def _gws_binary() -> str | None:
    override = os.getenv("JOEY_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Joey Agent")
