...
metadata:
  joey:
    ...
    related_skills: [claude-code, joey-agent]
---

# Codex CLI

Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Joey terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Joey itself, `model.provider: openai-codex` uses Joey-managed Codex
OAuth from `~/.joey/auth.json` after `joey auth add openai-codex`. For the
...
When invoking the Codex CLI from a Joey gateway/service context (for example,
