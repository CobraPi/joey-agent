...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _joey_env="${JOEY_HOME:-$HOME/.joey}/.env"; [ -f "$_joey_env" ] && grep -q "^GITHUB_TOKEN=" "$_joey_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_joey_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Joey Agent\",
    \"comments\": [
...
```bash
source "${JOEY_HOME:-$HOME/.joey}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Joey Agent. Code looks clean — good test coverage, no security concerns."
