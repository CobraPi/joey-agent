#!/usr/bin/env bash
# GitHub environment detection helper for Joey Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _joey_env="${JOEY_HOME:-$HOME/.joey}/.env"; [ -f "$_joey_env" ] && grep -q "^GITHUB_TOKEN=" "$_joey_env" 2>/dev/null; then
