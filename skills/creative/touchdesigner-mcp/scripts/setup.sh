...
JOEY_HOME_DIR="${JOEY_HOME:-$HOME/.joey}"
JOEY_CFG="${JOEY_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Joey config has twozero_td MCP entry ──
if [[ ! -f "$JOEY_CFG" ]]; then
    echo -e " ${FAIL} Joey config not found at ${JOEY_CFG}"
    manual_steps+=("Create ${JOEY_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$JOEY_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Joey config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Joey config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${JOEY_CFG} manually"); }
    manual_steps+=("Restart Joey session to pick up config change")
fi
