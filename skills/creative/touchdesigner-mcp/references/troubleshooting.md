...
## 5. Joey Configuration

### Config location

`$JOEY_HOME/config.yaml` (defaults to `~/.joey/config.yaml` when `JOEY_HOME` is unset)
...
Restart the Joey session for changes to take effect. The MCP connection is
