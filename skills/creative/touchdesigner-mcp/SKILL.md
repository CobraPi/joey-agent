...
metadata:
  joey:
    ...
    related_skills: [native-mcp, ascii-video, manim-video, joey-video]
...
```
Joey Agent -> MCP (Streamable HTTP) -> twozero.tox (port 40404) -> TD Python
```
...
```bash
bash "${JOEY_HOME:-$HOME/.joey}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to Joey config (if missing)
...
3. **Restart Joey session** to pick up the new MCP server
